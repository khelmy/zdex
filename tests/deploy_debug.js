...
function init_shared(privkey, zilliqa) {
  ...
  const zdex_a_code = fs.readFileSync('./contracts_debug/ZDExchange_a.scilla', 'utf8');
  ...
  const zdex_b_code = fs.readFileSync('./contracts_debug/ZDExchange_b.scilla', 'utf8');

  return [address, zdex_a_code, zdex_init, zdex_b_code, zdex_init];
}
...
function init_testnet() {
  ...
  const privkey = fs.readFileSync('./tests/privkey_testnet.txt', 'utf8');
...
async function deploy_all_v(network) {
  ...
  console.log("ZDExchange_a contract address: ");
  console.log(z_address);
  console.log("ZDExchange_b contract address: ");
